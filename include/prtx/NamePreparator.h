 ...
 */
class PRTX_EXPORTS_API NamePreparator : boost::noncopyable {
public:
	/**
	...
	typedef boost::shared_ptr<NamespaceInfo> NamespacePtr;
...
public:
	 ...
	 */
	std::wstring legalizedAndUniquifed(const std::wstring& name, uint32_t entity, const NamespacePtr& ns) {
		std::wstring s(name);
...
public:
	 ...
	 * @param [in] entity The entity of the name.
	 */
	void legalize(std::wstring& name, uint32_t entity);
	 ...
	 */
	void uniquify(std::wstring& name, uint32_t entity, const NamespacePtr& ns);
...
private:
	bool isIllegalFilenameCharacter(wchar_t ch);
