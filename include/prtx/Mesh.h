...
#ifdef _MSC_VER
#	ifdef PRT_CC_DBG
#		pragma message(__FILE__": disabled MSVC warnings 4231 4251 4275 4660")
#	endif
#	pragma warning(push)
...
class Mesh;
typedef boost::shared_ptr<Mesh> MeshPtr;		///< shared Mesh pointer
...
public:
	 ...
	 */
	const DoubleVector&			getUVCoords(size_t uvSet) const;
	/**
	 ...
	 * @param faceId Index of face.
	 * @returns pointer to array with vertex indices; size of array is getFaceCount(faceId).
	 */
 ...
 *
 */
class PRTX_EXPORTS_API MeshBuilder : public PRTXBuilder<Mesh> {
public:
	/**
	...
	/**
	 * Creates a new Mesh instance from the current state.
	 * @sa Mesh
	 * @param[out] warnings Optional pointer to a string to return warnings.
	 * @returns a valid pointer to a new Mesh instance. Throws an exception if mesh conditions are not met.
	 */
	Mesh* create(std::wstring* warnings = 0) const;
	/**
	 * Creates a new Mesh instance from the current state and resets the builder.
	 * @sa Mesh
	 * @param[out] warnings Optional pointer to a string to return warnings.
	 * @returns a valid pointer to a new Mesh instance. Throws an exception if mesh conditions are not met.
	 */
	Mesh* createAndReset(std::wstring* warnings = 0);
	...
	/**
	 * Adds a number of uv tuples (i.e. a number of coordinate tuples) to the normal coordinates array.
	 * @param uvSet  Index of uv set.
