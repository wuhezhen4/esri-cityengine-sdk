...
/**
 * @namespace prtx::EncodeOptionsAnnotations
 * Supported annotation types for encoder options. Typically these annotations are used
 * to programmatically construct a user interface from an EncoderInfo instance.
 *
 * \sa prt::EncoderInfo
 * \sa prt::createEncoderInfo
 */
namespace EncodeOptionsAnnotations {


const std::wstring ORDER		= L"@Order";		///< Defines a semantic ordering of the options (one double parameter).
 ...
 */
class PRTX_EXPORTS_API EncodeOptionsAnnotator : boost::noncopyable {
public:
...
public:
	...
	/**
	 * Adds an EncodeOptionsAnnotations::LABEL annotation.
	 */
	void setLabel(const std::wstring& optionKey, const std::wstring& label);

	/**
	 * Adds an EncodeOptionsAnnotations::DESCRIPTION annotation.
	 */
	void setDescription(const std::wstring& optionKey, const std::wstring& desc);

	/**
	 * Adds an EncodeOptionsAnnotations::ORDER annotation.
	 */
	void setOrder(const std::wstring& optionKey, const double& order);

	/**
	 * Adds an EncodeOptionsAnnotations::GROUP annotation.
	 */
	void setGroup(const std::wstring& optionKey, const std::wstring& groupName, const double& order);

	/**
	 * Adds an EncodeOptionsAnnotations::RANGE annotation with double values (= numeric range).
	 */
	void setRange(const std::wstring& optionKey, double min, double max);

	/**
	 * Adds an EncodeOptionsAnnotations::RANGE annotation with string values (= enumeration).
	 */
	void setEnumerants(const std::wstring& optionKey, const StringEnum& items);

	/**
	 * Adds an EncodeOptionsAnnotations::POINT2D annotation.
	 */
	void flagAsPoint2D(const std::wstring& optionKey);

	/**
	 * Adds an EncodeOptionsAnnotations::POINT3D annotation.
	 */
	void flagAsPoint3D(const std::wstring& optionKey);

	/**
	 * Adds an EncodeOptionsAnnotations::PERLAYER annotation with a string default value.
	 */
	void flagAsPerLayer(const std::wstring& optionKey, const std::wstring& layerDefault);

	/**
	 * Adds an EncodeOptionsAnnotations::PERLAYER annotation with a double default value.
	 */
	void flagAsPerLayer(const std::wstring& optionKey, const double& layerDefault);

	/**
	 * Adds an EncodeOptionsAnnotations::PERLAYER annotation with bool default value.
	 */
	void flagAsPerLayer(const std::wstring& optionKey, const bool& layerDefault);

	/**
	 * Adds an EncodeOptionsAnnotations::PERLAYER annotation with a in32_t default value.
	 */
	void flagAsPerLayer(const std::wstring& optionKey, const int32_t& layerDefault);

	/**
	 * Adds an EncodeOptionsAnnotations::HIDDEN annotation.
	 */
...
public:
	...
	private:
		OptionAnnotator& operator=(const OptionAnnotator&) {
			assert(false);
			return *this;
		}
