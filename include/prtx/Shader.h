...
class Shader;
typedef boost::shared_ptr<Shader> ShaderPtr;		///< shared Shader pointer
...
public:
	 ...
	 */
	enum KeyType { KT_INT, KT_BOOL, KT_FLOAT, KT_STRING, KT_TEXTURE };
 ...
 */
class PRTX_EXPORTS_API ShaderBuilder : public PRTXBuilder<Shader> {
public:
	ShaderBuilder();
	...
	Shader* create(std::wstring* warnings = 0) const;
	Shader* createAndReset(std::wstring* warnings = 0);
